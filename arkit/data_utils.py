...
import numpy as np
from arkit.scene_gen_utils import (
...
from arkit.utils import choice_without_replacement
...
class KeypointSceneBuffer(Dataset):
    def __init__(self, scene_path_pair_list: List[Tuple[str, str]], cfg: NamedTuple, device: torch.DeviceObjType):
        ...
        if os.path.exists(self.cfg.obj_list_file):
            with open(self.cfg.obj_list_file, 'r') as f:
                obj_dirs = sorted([s.strip() for s in f.readlines()])
                train_obj_dirs = sorted([s for s in obj_dirs if 'train' in s])
                test_obj_dirs = sorted([s for s in obj_dirs if 'test' in s])
        else:
            train_obj_dirs = sorted(glob(os.path.join(self.cfg.obj_root, "train", "*.ply")))
            test_obj_dirs = sorted(glob(os.path.join(self.cfg.obj_root, "test", "*.ply")))
        ...
        for scene_idx, (scene_path_pos, scene_path_pair_pos) in enumerate(tqdm(scene_path_pair_list)):
            ...
            for scene_name in scene_names_list:
                ...
                # NOTE: Unlike 3D-FRONT, we load 'positive' scenes which contain the original scan for real heterogeneous pairs as 'pair_pos'
                if scene_name == 'pair_pos':
                    if self.scene_pair_type == 'real_hetero':  # For real heterogeneous pairs, load 'positive' scenes as 'pair_pos' instead
                        load_scene_name = 'pos'
                    else:  # For other pair types, use the same scene to load
                        load_scene_name = scene_name
                else:
                    load_scene_name = scene_name  # For other scene types, use the same scene to load

                obj_trans = scene_triplet[load_scene_name + "_trans"]
                obj_rot = scene_triplet[load_scene_name + "_rot"]
                obj_scene_scales = scene_triplet[load_scene_name + "_obj_scene_scales"]

                scene_fp_pts = scene_triplet[load_scene_name + "_fp_points"]  # (N_fp_pts, 3)
                ...
                for obj_idx in range(scene_triplet[load_scene_name + "_obj_id"].shape[0]):
                    sample_path = os.path.join(
                        self.cfg.point_sample_root,
                        f"sample_{self.cfg.scene_sample_points}",
                        scene_triplet[load_scene_name + "_obj_id"][obj_idx] + ".npy"
                    )
                    scene_sample_pcd_np = np.load(sample_path)

                    scene_sample_pcd_np *= scene_triplet[load_scene_name + '_obj_scene_scales'][obj_idx]
                    scene_sample_pcd_np = scene_sample_pcd_np @ scene_triplet[load_scene_name + '_rot'][obj_idx].T + scene_triplet[load_scene_name + '_trans'][obj_idx]
                ...
                obj_sem_labels = scene_triplet[load_scene_name + "_obj_classes"][:, None]  # (N_obj, 1)
                ...
                scene_obj_paths = scene_triplet[load_scene_name + "_obj_path"]
                ...
                if self.query_point_type == 'obj_points':  # Object-proximal points
                    ...
                    if scene_name == 'neg':  # Sample from original positive object points for training
                        ...
                        grid_scene_name = load_scene_name
                    ...
                    # NOTE: This part is different from 3D-FRONT as the canonical pose annotation is different 
                    ...
                    query_points = generate_2d_grid_points(
                        num_grid_points,
                        scene_triplet[grid_scene_name + "_bbox_centroids"][:, [0, 1]],
                        scene_triplet[grid_scene_name + "_bboxes"][:, None, [0, 1]],
                        scene_triplet[grid_scene_name + "_rot"][:, [0, 2], :][:, :, [0, 1]],
                    ...
                    scene_min_x = scene_fp_pts[:, 0].min()
                    scene_max_x = scene_fp_pts[:, 0].max()
                    scene_min_y = scene_fp_pts[:, 2].min()
                    scene_max_y = scene_fp_pts[:, 2].max()
                    ...
                    init_in_polygon = check_in_polygon(scene_fp_pts[:scene_fp_pts.shape[0] // 2, [0, 2]], init_query_points)
                    ...
                    surplus_in_polygon = check_in_polygon(scene_fp_pts[:scene_fp_pts.shape[0] // 2, [0, 2]], surplus_query_points)
                    query_points = surplus_query_points[surplus_in_polygon]

                    # Add vertical points
                    max_vert_level = scene_fp_pts[:, 1].max() if force_up_margin is None else scene_fp_pts[:, 1].max() - force_up_margin
                    min_vert_level = scene_fp_pts[:, 1].min() if force_low_margin is None else scene_fp_pts[:, 1].min() + force_low_margin
                    vert_points = np.linspace(min_vert_level, max_vert_level, num_vert_split)
                    vert_points = np.repeat(vert_points, query_points.shape[0])[:, None]
...
def keybuff_collate_fn(batch_list):
    ...
    scene_pcd = {'pos': pos_pcd, 'pair_pos': pair_pos_pcd, 'neg': neg_pcd, 'scene_path': scene_path_dict}
...
def compute_box_3d(scale, transform, rotation):
    # Utility function for creating 3D boxes from ARKiT annotations
...
def build_dense_3d_scene(scene_path, scene_name, remove_inst_list=None, obj_return_type='mesh', num_mesh_samples=10000, remove_obj_mesh=True, add_bbox=False, add_inst_name=False, add_sem_name=False, return_annot_scene=False):
    ...
    # NOTE 4: If remove_obj_mesh == True, object meshes are removed from original scene mesh during visualization
    scene_triplet = np.load(scene_path)

    ARKIT_TO_TDFRONT = np.array([[1., 0., 0.], [0., 0., 1.], [0., 1., 0.]])  # Transformation matrix for 3D-FRONT style coordinates
    if scene_name + "_orig_scene_path" in scene_triplet.keys():  # Additionally load background mesh information
        orig_scene_path = scene_triplet[scene_name + "_orig_scene_path"][0]
        scene_id = os.path.basename(orig_scene_path.rstrip("/"))
        orig_mesh_path = os.path.join(orig_scene_path, scene_id + "_3dod_mesh.ply")
        orig_annot_path = os.path.join(orig_scene_path, scene_id + "_3dod_annotation.json")

        scene_mesh = o3d.io.read_triangle_mesh(orig_mesh_path)
        scene_mesh.vertices = o3d.utility.Vector3dVector(np.asarray(scene_mesh.vertices) @ ARKIT_TO_TDFRONT.T)

        # Apply global rotation for scenes that have been augmented
        if 'pair_pos_global_rot' in scene_triplet.keys():
            scene_mesh.vertices = o3d.utility.Vector3dVector(np.asarray(scene_mesh.vertices) @ scene_triplet['pair_pos_global_rot'].T)

        # Add reversed trianges for clear visualization
        scene_mesh.triangles = o3d.utility.Vector3iVector(np.concatenate([np.asarray(scene_mesh.triangles), np.asarray(scene_mesh.triangles)[:, [0, 2, 1]]], axis=0))

        with open(orig_annot_path, 'rb') as f:
            scene_annot = json.load(f)

        ceil_thres = np.quantile(np.asarray(scene_mesh.vertices)[:, 1], 0.25)  # Threshold value for cropping ceilings
        orig_bbox_list = []
        for annot_data in scene_annot['data']:
            # Save objects
            ...
            orig_bbox_list.append(oriented_bbox)
            if remove_obj_mesh:
                in_idx = oriented_bbox.get_point_indices_within_bounding_box(scene_mesh.vertices)
                scene_mesh.remove_vertices_by_index(in_idx)  # Keep mesh regions without object annotations

        scene_mesh_vertices = np.asarray(scene_mesh.vertices)
        ceil_idx = np.where(scene_mesh_vertices[:, 1] > ceil_thres)[0].tolist()
        scene_mesh.remove_vertices_by_index(ceil_idx)  # Remove ceiling mesh regions
    else:
        scene_mesh = None
    ...
    for obj_idx, scene_obj_path in enumerate(scene_obj_paths):
        if remove_inst_list is not None and obj_idx in remove_inst_list:
            continue
        tr_mesh = trimesh.load(scene_obj_path)
        ...
        if obj_return_type == 'mesh':
            o3d_mesh = o3d.io.read_triangle_mesh(scene_obj_path)
            canonical_vertices = np.asarray(o3d_mesh.vertices)
            transformed_vertices = canonical_vertices * obj_scene_scales[obj_idx].reshape(-1, 3)
            transformed_vertices = transformed_vertices @ obj_rot[obj_idx].T + obj_trans[obj_idx: obj_idx + 1]
    ...
    if scene_mesh is not None:  # Use mesh floors
        floorplan_model = scene_mesh
    else:  # Use wireframe
    ...
    if add_bbox:
        ...
        for obj_idx, obj_model in enumerate(obj_list):
            bbox = o3d.geometry.OrientedBoundingBox(orig_bbox_list[obj_idx])
...
def get_histogram_scene_pairs(scene_path_list, num_classes):
    # NOTE: For ARKiT scenes, multiple scans correspond to a same spot. Use metadata to remove such spots when performing histogram-based retrieval.
    scene_metadata = pd.read_csv("./arkit/scene_metadata.csv")
    scene_metadata = scene_metadata.values

    scene_id_arr = scene_metadata[:, 0].astype(int)
    visit_id_arr = scene_metadata[:, 1].astype(float)

    scene2visit = {}
    visit2scene = {}
    for scene_id, visit_id in zip(scene_id_arr, visit_id_arr):
        scene2visit[scene_id] = visit_id
        if not math.isnan(visit_id):
            if visit_id not in visit2scene.keys():
                visit2scene[visit_id] = []
            visit2scene[visit_id].append(str(scene_id))

    covisit_scene_map = {str(scene_id): [] for scene_id in scene_id_arr}
    for scene_id in scene_id_arr:
        if not math.isnan(scene2visit[scene_id]):
            visit_id = scene2visit[scene_id]
            covisit_scene_map[str(scene_id)].extend(visit2scene[visit_id])
    ...
    for path_idx, path in tqdm(enumerate(scene_path_list), desc="Extracting Histograms for Matching", total=len(scene_path_list)):
        ...
        hist_vec = np.bincount(scene_info['pos_obj_classes'], minlength=num_classes).astype(float)
        full_semantic_hist[path_idx] = hist_vec
        scene_id = os.path.basename(scene_info['pos_orig_scene_path'][0])
        scene_id2path_idx[scene_id] = path_idx
        path_idx2scene_id[path_idx] = scene_id
    ...
    for path_idx, path in tqdm(enumerate(scene_path_list), desc="Pair Generation", total=len(scene_path_list)):
        ...
        # Make distances to scenes from same scan point as infinity
        scene_id = path_idx2scene_id[path_idx]
        covisit_scene_ids = covisit_scene_map[scene_id]
        for cov_scene_id in covisit_scene_ids:
            if cov_scene_id in scene_id2path_idx.keys():
                hist_dist[scene_id2path_idx[cov_scene_id]] = np.inf
...
if __name__ == '__main__':
    ...
    parser.add_argument("--feat_pca_root", help="Root folder containing PCA components for point feature dimension reduction (currently used for Vector Neurons)", default="./data/arkit_scenes_pca/")
    ...
    # Training configs
    parser.add_argument("--scene_root", help="Root directory containing scene data", type=str, default='./data/arkit_large_noise/')
    ...
    parser.add_argument("--num_cache_query", help="Number of query point locations to cacher per scene", default=50, type=int)
    parser.add_argument("--num_sample_query", help="Number of query points to sample per scene", default=50, type=int)
    ...
    test_scene_path_pair_list = [(path, path) for path in full_scene_path_list if 'test' in path]
    test_scene_path_pair_list = test_scene_path_pair_list[:args.scene_buffer_size]

    keypoint_buffer = KeypointSceneBuffer(
        scene_path_pair_list=test_scene_path_pair_list,
        cfg=args,
        device=device
    )
