...
class KeyPointFeatureField(AbstractFeatureField):
    def __init__(self, cfg, device: torch.device):
        ...
        self.semantics_emb_type = getattr(cfg, 'semantics_emb_type', 'class_vector')
        self.semantics_emb_size = getattr(cfg, 'semantics_emb_size', 32)

        if self.semantics_emb_size != 0:
            if self.semantics_emb_type == 'class_vector':
    ...
    def forward(self, query_pcd: Pointclouds, scene_pcd: Pointclouds, **kwargs):
        ...
        if self.semantics_emb_size != 0:
            if getattr(self, "semantics_emb_type", 'class_vector') == 'class_vector':
...
class KeyPointSimpleField(AbstractFeatureField):
    ...
    def forward(self, query_pcd: Pointclouds, scene_pcd: Pointclouds):
        ...
        dist_softmax_mtx = torch.softmax(-agg_dists_flatten / self.agg_exp_temp, dim=1)  # (N_batch * N_query, K, 1)
        dist_softmax_mtx[torch.isnan(dist_softmax_mtx)] = 0.  # Zero out NaN values as they will all be set to zero from exponentials
        dist_wgt_mtx = dist_softmax_mtx * torch.exp(-agg_dists_flatten / self.agg_exp_temp)  # (N_batch * N_query, K, 1)
