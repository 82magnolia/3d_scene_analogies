...
class PointMatcher:
    ...
    def find_transforms(self, global_transform_list: List[Dict[str, torch.Tensor]], global_inst_match_list: List[Dict[str, torch.Tensor]],
            tgt_query_pcd: Pointclouds, ref_query_pcd: Pointclouds, tgt_scene_pcd: Pointclouds, ref_scene_pcd: Pointclouds, **kwargs):
        ...
        for scene_idx in range(num_batch_scenes):
            ...
            for g_idx, global_transform in enumerate(tqdm(global_transform_list[scene_idx], desc=f"Local Matching (Scene {scene_idx + 1}, {len(global_transform_list[scene_idx])} Transforms)")):
                ...
                with torch.no_grad():
                    ...
                    tgt_feats = self.local_feature_field(tgt_query_pcd, tgt_scene_pcd, **pair_pos_args)[0]  # (N_query, D_feat)
                    ref_feats = self.local_feature_field(ref_query_pcd, ref_scene_pcd, **pos_args)[0]  # (N_query, D_feat)
                ...
                # Generate interpolator from initial estimation
                if isinstance(global_transform, RBFInterpolator):  # Initial estimation is an RBF interpolator
                    warp_query = global_transform(tgt_query_points)
            ...
            if len(local_transform_list[scene_idx]) != 0:
                ...
                if getattr(self.cfg, "local_threshold_method", "valid") == "valid":
                ...
                else:  # Keep local transforms within a margin from the minimum value
                    local_transform_list[scene_idx] = [local_transform_list[scene_idx][l_idx] for l_idx in topk_list if cost_arr[l_idx] < cost_arr.min() + self.cfg.local_margin_thres]
                    local_inst_match_list[scene_idx] = [local_inst_match_list[scene_idx][l_idx] for l_idx in topk_list if cost_arr[l_idx] < cost_arr.min() + self.cfg.local_margin_thres]
