...
from threed_front.atiss_utils.threed_front_scene import Asset, ModelInfo, Room, ThreedFutureModel, \
    ThreedFutureExtra


def parse_threed_front_scenes(
    dataset_directory, path_to_model_info, path_to_models,
    path_to_room_masks_dir=None, num_load_layout=-1
):
    if os.getenv("PATH_TO_SCENES"):
        ...
        if num_load_layout > 0:
            path_to_scene_layouts = path_to_scene_layouts[:num_load_layout]

        scenes = []
        unique_room_ids = set()
        # Start parsing the dataset
        print("Loading 3d front dataset ", end="")
...
def parse_threed_future_models(
    ...
    if os.getenv("PATH_TO_3D_FUTURE_OBJECTS"):
        ...
        # Start parsing the dataset
        print("Loading 3d future dataset ", end="")
        for i, m in enumerate(path_to_scene_layouts):
            with open(m) as f:
                data = json.load(f)
                # Parse the furniture of the scene
