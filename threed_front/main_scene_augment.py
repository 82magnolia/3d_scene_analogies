...
import subprocess
from threed_front.data_utils import build_dense_3d_scene, generate_obj_surface_points
...
if __name__ == '__main__':
    ...
    parser.add_argument("--fp_max_noise", help="Maximum noise level for floorplan points", default=0.3, type=float)
    ...
    for scene_idx in trange(num_scenes):
        ...
        for p_idx in fp_perturb_idx_list:
            ...
            if np.abs(np.dot(dir_vec, np.array([1, 0, 0]))) < 0.1:  # Edge perpendicular to x-axis
                noise_dim = 0
            elif np.abs(np.dot(dir_vec, np.array([0, 0, 1]))) < 0.1:  # Edge perpendicular to z-axis
                noise_dim = 2
            else:
                noise_dim = random.choice([0, 2])

            fp_0[noise_dim] += noise_val
            fp_1[noise_dim] += noise_val
        ...
        for k in augment_scene_triplet.keys():
            if k not in ['pair_pos_idx_map', 'pair_pos_fp_points', 'pair_pos_fp_lines']:
        ...
        pose_perturb_scene_triplet = {k: np.copy(v[pose_perturb_obj_idx_list]) for k, v in global_augment_scene_triplet.items() if k not in ['pair_pos_idx_map', 'pair_pos_fp_points', 'pair_pos_fp_lines']}
            ...
            for obj_add_idx in np.random.permutation(match_obj_inst_labels):
                ...
                if placeable:
                    ...
                    for k in match_add_triplet.keys():
                        if k not in ['pair_pos_idx_map', 'pair_pos_trans', 'pair_pos_fp_points', 'pair_pos_fp_lines']:  # Centroids and translations are modified after placement
